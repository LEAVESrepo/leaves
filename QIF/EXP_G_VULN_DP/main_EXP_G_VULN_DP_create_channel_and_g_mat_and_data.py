...
G_OBJ = EXP_G_VULN_DP_FOLDER + "G_OBJ/"
...
def create_single_dataset(len_rows):
    list_targets = []
    list_features = []
    ...
    dataset = np.column_stack((array_features, array_targets))
...
def main_EXP_G_VULN_DP_create_channel_and_g_mat_and_data():
    ...
    for size_list_iterator in range(len(TRAINING_SET_SIZE)):
        ...
        for train_iteration in tqdm(range(TRAIN_ITERATIONS)):
            ...
            tr = create_single_dataset(training_set_size)
            val = create_single_dataset(validation_set_size)
    ...
    for test_iteration in tqdm(range(TEST_ITERATIONS)):
        ...
        ts = create_single_dataset(test_set_size)
        pn.to_pickle(obj=ts, path=DATA_FOLDER_TEST + "test_set_" + str(test_iteration) + ".pkl",
                     protocol=2)

    g_mat = create_g_mat()
    g_mat_rows = np.arange(start=0, stop=len(real_counts), step=1)
    g_mat_cols = np.arange(start=0, stop=len(real_counts), step=1)

    pn.to_pickle(obj=g_mat, path=G_OBJ + "g_mat.pkl", protocol=2)
    pn.to_pickle(obj=g_mat_rows, path=G_OBJ + "g_mat_rows.pkl", protocol=2)
    pn.to_pickle(obj=g_mat_cols, path=G_OBJ + "g_mat_cols.pkl", protocol=2)
