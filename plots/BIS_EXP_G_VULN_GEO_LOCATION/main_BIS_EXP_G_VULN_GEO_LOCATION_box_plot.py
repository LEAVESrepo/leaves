...
def plot_box_plot_BIS_EXP_G_GEO_LOCATION_GUESSES_new(df, title, y, save_fig=False):
    # print df.columns.values
    fig, ax1 = plt.subplots()
    ...
    ax1.annotate('dispersion: 0.033', xy=(-0.405, 0.43), size=8)
    ax1.annotate('total error: 0.498', xy=(-0.405, 0.41), size=8)
    plt.plot([-0.395, -0.005], [0.497, 0.497], linewidth=.5, color='black')

    ax1.annotate('dispersion: 0.038', xy=(0.005, 0.52), size=8)
    ax1.annotate('total error: 0.590', xy=(0.005, 0.50), size=8)
    plt.plot([0.005, 0.395], [0.589, 0.589], linewidth=.5, color='black')

    ax1.annotate('dispersion: 0.032', xy=(0.505, 0.21), size=8)
    ax1.annotate('total error: 0.283', xy=(0.505, 0.19), size=8)
    plt.plot([0.605, 0.995], [0.281, 0.281], linewidth=.5, color='black')

    ax1.annotate('dispersion: 0.016', xy=(1.005, 0.33), size=8)
    ax1.annotate('total error: 0.278', xy=(1.005, 0.31), size=8)
    plt.plot([1.005, 1.395], [0.277, 0.277], linewidth=.5, color='black')

    ax1.annotate('dispersion: 0.013', xy=(1.4, 0.14), size=8)
    ax1.annotate('total error: 0.082', xy=(1.4, 0.12), size=8)
    plt.plot([1.605, 1.995], [0.081, 0.081], linewidth=.5, color='black')

    ax1.annotate('dispersion: 0.015', xy=(1.9, 0.18), size=8)
    ax1.annotate('total error: 0.121', xy=(1.9, 0.16), size=8)
    plt.plot([2.005, 2.3955], [0.120, 0.120], linewidth=.5, color='black')
    bp = sns.boxplot(x="number_of_training_samples", y=y, hue="hue", data=df,
                     ax=ax1,
                     linewidth=.5,
                     palette=sns.xkcd_palette(colors),
...
def main_BIS_EXP_G_VULN_GEO_LOCATION_box_plot():
    #################################################  REMAPPING KNN  ######################################################

    success, local_folder = download_from_remote.download_directory_from_server(
        remote_obj='/home/comete/mromanel/MILES_EXP/BIS_EXP_GEO_LOCATION_QIF_LIB_SETTING/RESULT_FOLDER_REMAPPING/KNN/KNN*.pkl'
    )
    colnames = None
    mat_final = None
    ...
    success, local_folder = download_from_remote.download_directory_from_server(
        remote_obj='/home/comete/mromanel/MILES_EXP/BIS_EXP_GEO_LOCATION_QIF_LIB_SETTING/RESULT_FOLDER_REMAPPING/model_100_300_500/'
                   'ANN*.pkl'
    )
    colnames = None
    mat_final = None
    ...
    plot_box_plot_BIS_EXP_G_GEO_LOCATION_GUESSES_new(df=mat_final_df_cpy,
                                                     title="",
                                                     y="ANN_Rf_values",
                                                     save_fig=True)
