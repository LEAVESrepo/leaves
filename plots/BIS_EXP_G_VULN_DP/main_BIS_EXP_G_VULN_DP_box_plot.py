...
def plot_box_plot_BIS_EXP_G_VULN_DP_new(df, title, y, save_fig=False):
    # print df.columns.values
    fig, ax1 = plt.subplots()
    colors = ["windows blue", "faded green"]  # ["windows blue", "faded green", "red"]
    ...
    #   plot real error baseline
    # plt.plot([-100000, 100000], [0.108, 0.108], linewidth=2, color='black', linestyle='dashed')
    ax1.annotate('dispersion: 0.025', xy=(-0.425, 0.003), size=8)
    ax1.annotate('total error: 0.059', xy=(-0.425, 0), size=8)
    plt.plot([-0.395, -0.005], [0.053, 0.053], linewidth=.5, color='black')

    ax1.annotate('dispersion: 0.006', xy=(-0.025, 0.021), size=8)
    ax1.annotate('total error: 0.040', xy=(-0.025, 0.018), size=8)
    plt.plot([0.005, 0.395], [0.040, 0.040], linewidth=.5, color='black')

    ax1.annotate('dispersion: 0.008', xy=(0.505, 0.040), size=8)
    ax1.annotate('total error: 0.012', xy=(0.505, 0.037), size=8)
    plt.plot([0.605, 0.995], [0.008, 0.008], linewidth=.5, color='black')

    ax1.annotate('dispersion: 0.004', xy=(1.005, 0.013), size=8)
    ax1.annotate('total error: 0.027', xy=(1.005, 0.010), size=8)
    plt.plot([1.005, 1.395], [0.026, 0.026], linewidth=.5, color='black')

    ax1.annotate('dispersion: 0.003', xy=(1.53, 0.04), size=8)
    ax1.annotate('total error: 0.005', xy=(1.53, 0.037), size=8)
    plt.plot([1.605, 1.995], [0.004, 0.004], linewidth=.5, color='black')

    ax1.annotate('dispersion: 0.005', xy=(1.9, 0.048), size=8)
    ax1.annotate('total error: 0.022', xy=(1.9, 0.045), size=8)
    plt.plot([2.005, 2.3955], [0.022, 0.022], linewidth=.5, color='black')
    bp = sns.boxplot(x="number_of_training_samples", y=y, hue="hue", data=df,
                     ax=ax1,
                     linewidth=.5,
                     palette=sns.xkcd_palette(colors),
...
def main_EXP_G_VULN_MULTIPLE_GUESSES_box_plot():
    ...
    success, local_folder = download_from_remote.download_directory_from_server(
        remote_obj='/home/comete/mromanel/MILES_EXP/BIS_EXP_G_VULN_DP_FOLDER/RESULT_FOLDER_REMAPPING/KNN/KNN*.pkl'
    )
    colnames = None
    mat_final = None
    ...
    success, local_folder = download_from_remote.download_directory_from_server(
        remote_obj='/home/comete/mromanel/MILES_EXP/BIS_EXP_G_VULN_DP_FOLDER/RESULT_FOLDER_REMAPPING/model_200_ep500/ANN*.pkl'
    )
    colnames = None
    mat_final = None
    for filename in os.listdir(local_folder):
    ...
    plot_box_plot_BIS_EXP_G_VULN_DP_new(df=mat_final_df_cpy,
                                        title="",
                                        y="ANN_Rf_values",
                                        save_fig=True)
