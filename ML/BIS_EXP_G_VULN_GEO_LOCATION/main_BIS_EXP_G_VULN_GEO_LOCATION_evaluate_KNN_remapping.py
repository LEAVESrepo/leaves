...
EXP_G_VULN_GEO_LOCATION_FOLDER = "/home/comete/mromanel/MILES_EXP/BIS_EXP_GEO_LOCATION_QIF_LIB_SETTING/"

RESULT_FOLDER = EXP_G_VULN_GEO_LOCATION_FOLDER + "RESULT_FOLDER_REMAPPING/"
utilities.createFolder(RESULT_FOLDER)

RESULT_FOLDER = RESULT_FOLDER + "KNN/"
utilities.createFolder(RESULT_FOLDER)

DATA_FOLDER = EXP_G_VULN_GEO_LOCATION_FOLDER + "DATA_FOLDER/"

G_MATRIX_PATH = EXP_G_VULN_GEO_LOCATION_FOLDER + "G_OBJ/g_mat.pkl"
G_MATRIX_ROWS_PATH = EXP_G_VULN_GEO_LOCATION_FOLDER + "G_OBJ/g_mat_rows.pkl"
G_MATRIX_COLS_PATH = EXP_G_VULN_GEO_LOCATION_FOLDER + "G_OBJ/g_mat_cols.pkl"
...
def main_EXP_G_VULN_GEO_LOCATION_evaluate_KNN_remapping():
    ...
    if 'KNN' in APPROACHES:
        ...
        for size_list_iterator in range(len(TRAINING_SET_SIZE)):
            ...
            for train_iteration in range(TRAIN_ITERATIONS):
                ...
                O_train_unq_coord = np.zeros((O_train_unq.shape[0], 2))

                for i_ter in range(O_train_unq.shape[0]):
                    y = int(O_train_unq[i_ter] / NUM_CELLS)
                    x = O_train_unq[i_ter] - (NUM_CELLS * y)
                    O_train_unq_coord[i_ter, 0] = x
                    O_train_unq_coord[i_ter, 1] = y

                knn_classifier = KNN_express.KNN_express_EstimatorManager(
                    observed_samples=np.column_stack((O_train_unq_coord, Z_train_mod)),
                    k_neighbors=k_neighbors)

                # knn_classifier = KNN_express.KNN_express_EstimatorManager(
                #     observed_samples=np.column_stack((O_train, Z_train)),
                #     k_neighbors=int(np.log(len(O_train))))

                print "\n\n\n\n\n\n\n\n#################################  test_size: " + str(
                    test_set_size) + " ################################"
                KNN_file_Rf_ANN_g_leak = open(RESULT_FOLDER + "/KNN_" + str(training_set_size) + "_training_and_" + str(
                ...
                for test_iterator in range(TEST_ITERATIONS_BEG, TEST_ITERATIONS_END):
                    ...
                    O_test_unq_coord = np.zeros((O_test_unq.shape[0], 2))

                    for i_ter in range(O_test_unq.shape[0]):
                        y = int(O_test_unq[i_ter] / NUM_CELLS)
                        x = O_test_unq[i_ter] - (NUM_CELLS * y)
                        O_test_unq_coord[i_ter, 0] = x
                        O_test_unq_coord[i_ter, 1] = y

                    ind, pred = knn_classifier.predict_KNN_KDT(observables_test=O_test_unq_coord,
                                                               enhanced=ENHANCED)
                    ...
                    Rf_KNN_g_leak = g_vuln_computation.compute_g_vuln_with_remapping(
                        final_mat=final_mat,
